...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Real Weather Data from wttr.in (free, no API key required)
    async function updateWeather() {
        try {
            // Get weather for Nashville, TN (center of Tennessee)
            ...
            const weatherIconMap = {
                ...
                'heavy rain at times': 'fa-cloud-rain',
                ...
                'moderate or heavy rain shower': 'fa-cloud-rain',
                'torrential rain shower': 'fa-cloud-rain',
            ...
            const icon = weatherIconMap[conditionKey] || 'fa-cloud';
            ...
            if (tempElement) {
                tempElement.textContent = `${temp}°F - ${condition}`;
            }
            if (windElement) {
            ...
            console.log('Weather updated with real data:', { temp, condition, windSpeed, visibility });
            
        } catch (error) {
            console.log('Weather API error, using fallback data:', error);
            // Fallback to a single realistic weather condition if API fails
            const fallbackWeather = { temp: '72°F', wind: '8 mph', visibility: '10 mi', icon: 'fa-cloud-sun', condition: 'Partly Cloudy' };
            ...
            if (tempElement) {
                tempElement.textContent = `${fallbackWeather.temp} - ${fallbackWeather.condition}`;
            }
            if (windElement) {
                windElement.textContent = fallbackWeather.wind;
            }
            if (visibilityElement) {
                visibilityElement.textContent = fallbackWeather.visibility;
            }
            
            const weatherIcon = document.querySelector('.weather-widget i');
            if (weatherIcon) {
                weatherIcon.className = `fas ${fallbackWeather.icon}`;
            }
        }
    }
